...
description: "Review an implemented user story scenario (or all scenarios) against its breakdown files (prod, tech, design, testing) before QA. Emits ready-for-qa when no blocking findings remain. Optionally skip unchanged roles with skip=<comma-separated roles>."
category: Workflow
tags: [workflow, review, scenario, breakdown, development]
---
...
The workflow runs after implementation and before testing:

```
/prd → /breakdown-task → [implement] → [review-code] → /qa-test-script → /qa-test-run → /qa-report
```

`ready-for-qa` is emitted only when zero blocking findings remain across all included role docs.
