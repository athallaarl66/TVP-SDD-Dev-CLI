...
// AI tool directories
const AI_DIRS = ['.devin', '.opencode', '.claude', '.antigravity', '.agent', '.hermes'];
const AI_SUBDIRS = ['skills', 'workflows', '_templates'];
const LEGACY_WINDSURF_DIR = '.windsurf';

// Antigravity alias mapping (antigravity can also install to .agent)
...
// Get target directories for a given AI tool (handles aliases)
function getTargetDirectories(aiDir) {
  if (ANTIGRAVITY_ALIASES[aiDir]) {
    return ANTIGRAVITY_ALIASES[aiDir];
  }
  return [aiDir];
}

// Check if directory is an alias of another
function isAlias(dir) {
  for (const [key, aliases] of Object.entries(ANTIGRAVITY_ALIASES)) {
    if (aliases.includes(dir) && dir !== key) {
      return key;
    }
  }
  return null;
