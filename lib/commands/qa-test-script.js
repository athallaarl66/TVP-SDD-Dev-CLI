...
const { resolveFeatureName } = require('../utils/prompt');
...
const FRAMEWORK_DETECTORS = [
  ...
  { name: 'Go', files: ['go.mod'], check: () => true },
...
const FRAMEWORKS = {
  ...
  gotest: { name: 'Go testing', ext: '_test.go' }
};
...
function getFrameworkChoices(detectedTypes) {
  ...
  if (detectedTypes.length > 0) {
    choices.push(new inquirer.default.Separator('--- Detected frameworks ---'));
    ...
    for (const type of detectedTypes) {
      ...
      else if (type.includes('Go')) choices.push({ name: `🔷 ${type}`, value: 'gotest' });
      else if (type.includes('JavaScript')) choices.push({ name: `🟨 ${type} (Jest)`, value: 'jest' });
    }
  }
  
  // Add other options
  choices.push(new inquirer.default.Separator('--- Other frameworks ---'));
  ...
  if (!detectedTypes.some(t => t.includes('Go'))) choices.push({ name: '🔷 Go (testing)', value: 'gotest' });
  if (!detectedTypes.some(t => t.includes('JavaScript'))) choices.push({ name: '🟨 JavaScript/TypeScript (Jest)', value: 'jest' });
  
  return choices;
}

// Pick the first framework that matches detected project types, else playwright.
function pickDetectedFramework(detectedTypes) {
  if (detectedTypes.some((t) => t.includes('PHP'))) return 'phpunit';
  if (detectedTypes.some((t) => t.includes('.NET'))) return 'xunit';
  if (detectedTypes.some((t) => t.includes('JavaScript'))) return 'jest';
  if (detectedTypes.some((t) => t.includes('Java'))) return 'junit';
  if (detectedTypes.some((t) => t.includes('Python'))) return 'pytest';
  if (detectedTypes.some((t) => t.includes('Go'))) return 'gotest';
  return 'playwright';
}

function registerQATestScriptCommand(program, getOpts) {
  program
    ...
    .action(async (featureName) => {
      try {
        ...
        featureName = await resolveFeatureName(featureName, {
          ...
          usage: 'sdd-gen /qa-test-script <featureName>'
        });
        ...
        // Resolve framework: config > detected > playwright in --yes mode;
        // otherwise prompt (config choice preselected).
        const config = readConfig(paths.PROJECT_DIR);
        let framework;
        if (opts.yes) {
          framework = config.testFramework || pickDetectedFramework(detectedTypes);
        } else {
          const choices = getFrameworkChoices(detectedTypes);
          const { framework: fw } = await inquirer.default.prompt([
            {
              type: 'list',
              name: 'framework',
              message: 'What type of test script do you want to generate?',
              choices: choices,
              default: config.testFramework || undefined
            }
          ]);
          framework = fw;
        }
        writeConfig({ testFramework: framework, lastFeature: featureName }, paths.PROJECT_DIR);
        ...
        if (framework === 'playwright') {
          ...
          // Go convention: tests/feature_name_test.go
          testFileName = `${featureName.replace(/-/g, '_')}_test.go`;
          testScriptPath = path.join(paths.TESTS_DIR, testFileName);
        } else {
          // Jest (JS/TS)
          testFileName = `${featureName}${frameworkInfo.ext}`;
        ...
        switch (framework) {
          ...
          case 'pytest':
            ...
            break;
          case 'gotest':
            console.log(chalk.yellow(`1. Run 'go test -v -run Test${featureName.charAt(0).toUpperCase() + featureName.slice(1).replace(/-([a-z])/g, (g) => g[1].toUpperCase())} ./${featureName.replace(/-/g, '_')}' to run the test`));
            break;
          case 'jest':
