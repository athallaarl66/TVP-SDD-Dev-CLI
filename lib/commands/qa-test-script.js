...
function registerQATestScriptCommand(program) {
  program
    ...
    .description('Generate test script (Playwright or Unit) from SPEC_TEST document')
    .action(async (featureName) => {
      try {
        ...
        let testScript;
        if (framework === 'playwright') {
          testScript = generatePlaywrightScript(featureName, specTestContent);
        } else {
          testScript = generateUnitTestScript(featureName, specTestContent, framework);
        }
        ...
        console.log(chalk.green(`✅ ${frameworkInfo.name} test script created: ${testScriptPath}`));
