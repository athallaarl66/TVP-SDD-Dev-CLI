...
function registerInitCommand(program, getOpts) {
  program
    ...
    .action(async (featureName) => {
      try {
        ...
        const templates = [
          ...
          { name: 'Spec Test', template: 'testing-template.md', docsOutput: `${featureName}-spec-test.md`, docsDir: 'test-reports' },
        ...
        for (const doc of templates) {
          ...
          const filledContent = await fillDocTemplate(templateContent, featureName, paths.PROJECT_DIR);
          ...
          if (doc.docsDir === 'test-reports') {
            ...
            await fs.writeFile(docsOutputPath, filledContent, 'utf-8');
            console.log(chalk.green(`✅ ${doc.name} created: ${docsOutputPath}`));
          } else {
            // Generate to docs/features/{featureName}-{doc.docsOutput}.md
            const docsOutputPath = path.join(paths.DOCS_FEATURES_DIR, `${featureName}-${doc.docsOutput}`);
            await fs.writeFile(docsOutputPath, filledContent, 'utf-8');
            console.log(chalk.green(`✅ ${doc.name} created: ${docsOutputPath}`));
          }
        }
        ...
        console.log(chalk.yellow(`3. Run: sdd-gen /qa-test-script ${featureName}`));
        console.log(chalk.yellow(`4. Run: sdd-gen /qa-test-run ${featureName}\n`));
