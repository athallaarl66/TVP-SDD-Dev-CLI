...
const { getCliVersion } = require('../utils/version');

function registerInstallCommand(program, getOpts) {
  program
    ...
    .action(async () => {
      try {
        ...
        // Check for legacy .windsurf first (skip migration prompt in --yes)
        if (!opts.yes) {
        ...
        let selectedTools;
        if (opts.yes) {
          selectedTools = ['.devin', '.opencode', '.claude', '.antigravity', '.hermes'];
        } else {
          const { selectedTools: sel } = await inquirer.default.prompt([
            {
              type: 'checkbox',
              name: 'selectedTools',
...
╚══██╔══╝██║   ██║██╔══██╗      ██╔════╝██╔══██╗██╔══██╗
   ...
   ╚═╝     ╚═══╝  ╚═╝            ╚══════╝╚═════╝ ╚═════╝
                  ...
                  D E V  ·  C L I
`));
        console.log(chalk.gray(`authored by Talapvnk · version ${await getCliVersion(paths.CLI_DIR)} · unlicensed\n`));
