  ...
  program
    ...
    .action(async (featureName) => {
      try {
        ...
        const testScriptPath = path.join(paths.TESTS_DIR, `${featureName}.spec.ts`);
        const packageJsonPath = path.join(paths.PROJECT_DIR, 'package.json');
        
        // Check if test script exists
        if (!(await fs.pathExists(testScriptPath))) {
          console.error(chalk.red(`Error: ${featureName}.spec.ts not found in tests directory`));
        ...
        packageJson.scripts[scriptName] = `npx playwright test tests/${featureName}.spec.ts`;
        ...
        console.log(chalk.yellow(`Or run directly: npx playwright test tests/${featureName}.spec.ts`));
